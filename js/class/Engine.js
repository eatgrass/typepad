define([
  ...
  class Engine {
    ...
    applyConfig() {
      ...
      $("input[type=checkbox]#historyListMode").checked =
        this.config.isHistoryInListMode;
      ...
      if (
        // 1. ArticleType.word 时
        this.config.articleType === ArticleType.word
      ) {
        this.arrayWordAll =
          Article[this.config.articleIdentifier || "CET4"].getWordsArray();
      ...
      } // 2. ArticleType.phrase 时
      else if (this.config.articleType === ArticleType.phrase) {
        this.arrayWordAll =
          Article[this.config.articleIdentifier].getPhraseArray();
        if (this.config.count === "ALL") {
          this.arrayWordDisplaying = this.arrayWordAll;
        } else {
          this.arrayWordDisplaying = this.arrayWordAll.slice(
    ...
    prevChapter() {
      if (this.config.chapter !== 1) {
        if (this.config.articleType === ArticleType.word) {
          ...
          // 2. ArticleType.phrase
          this.arrayWordDisplaying = this.arrayWordAll.slice(
            this.config.count * (this.config.chapter - 2),
            this.config.count * (this.config.chapter - 1),
          ); // 截取当前需要显示的数组段
          this.currentWords = this.arrayWordDisplaying.join(" ");
        } // 3. ArticleType.others
        else {
          this.currentWords = this.currentOriginWords
            .slice(
              this.config.count * (this.config.chapter - 2),
    ...
    loadArticleOptions() {
      ...
      for (const [key, entity] of Object.entries(ArticleType)) {
        if (typeof entity !== "function") {
          // 除去 function 对象
          sortedArticles[key] = articleArray.filter(
            (item) => item.type === key,
          );
        }
      }
      ...
      for (const [articleTypeName, articleArray] of Object.entries(
        sortedArticles,
      )) {
        let tempOptionHtml = "";
        ...
        articleArray.forEach((item) => {
          switch (articleTypeName) {
            case ArticleType.customize:
              tempOptionHtml += `<option value="${item.value}">${
                this.config.customizedTitle || "未定义"
              }</option>`;
              break;
            case ArticleType.word:
              tempOptionHtml += `<option value="${item.value}">${item.name} - ${
                item.getWordsArray().length
              }词</option>`;
              break;
            default:
    ...
    getCurrentCETWordTranslation(length) {
      ...
      this.arrayWordDisplaying.forEach((item) => {
        ...
        if (length < afterString.length && length > tempString.length) {
          ...
          translationPanel.setAttribute(
            "href",
            `https://www.youdao.com/result?word=${item.word}&lang=en`,
          );
    ...
    finish() {
      ...
      this.record.codeLength = Number(
        (this.keyCount.all / this.correctWordsCount).toFixed(2),
      );
      this.record.speed = Number(
        ((this.correctWordsCount / this.duration) * 1000 * 60).toFixed(2),
      );
      ...
      this.score[this.config.articleType].hitRateAve =
        (this.score[this.config.articleType].keyCount /
          this.score[this.config.articleType].timeCost) *
        1000;
      // 击键 - 最小值
      ...
      this.score[this.config.articleType].codeLengthAve =
        this.score[this.config.articleType].keyCount /
        this.score[this.config.articleType].wordCount;
      // 码长 - 最小值
      if (this.score[this.config.articleType].codeLengthMin === 0) {
        this.score[this.config.articleType].codeLengthMin =
      ...
      if (
        this.score[this.config.articleType].codeLengthMax <
        this.record.codeLength
      ) {
        this.score[this.config.articleType].codeLengthMax =
          this.record.codeLength;
      ...
      // HIT RATE FILTER
      if (this.record.hitRate >= 0 && this.record.hitRate < 2) {
        this.score[this.config.articleType].hitRate1++;
      } else if (this.record.hitRate >= 2 && this.record.hitRate < 3) {
        this.score[this.config.articleType].hitRate2++;
      } else if (this.record.hitRate >= 3 && this.record.hitRate < 4) {
        this.score[this.config.articleType].hitRate3++;
      } else if (this.record.hitRate >= 4 && this.record.hitRate < 5) {
        this.score[this.config.articleType].hitRate4++;
      } else if (this.record.hitRate >= 5 && this.record.hitRate < 6) {
        this.score[this.config.articleType].hitRate5++;
      } else if (this.record.hitRate >= 6 && this.record.hitRate < 7) {
        this.score[this.config.articleType].hitRate6++;
      } else if (this.record.hitRate >= 7 && this.record.hitRate < 8) {
        this.score[this.config.articleType].hitRate7++;
      } else if (this.record.hitRate >= 8 && this.record.hitRate < 9) {
        this.score[this.config.articleType].hitRate8++;
      } else if (this.record.hitRate >= 9 && this.record.hitRate < 10) {
        this.score[this.config.articleType].hitRate9++;
      } else if (this.record.hitRate >= 10 && this.record.hitRate < 11) {
        this.score[this.config.articleType].hitRate10++;
      } else if (this.record.hitRate >= 11 && this.record.hitRate < 12) {
        this.score[this.config.articleType].hitRate11++;
      } else if (this.record.hitRate >= 12 && this.record.hitRate < 13) {
        this.score[this.config.articleType].hitRate12++;
      } else if (this.record.hitRate >= 13 && this.record.hitRate < 14) {
        this.score[this.config.articleType].hitRate13++;
      } else if (this.record.hitRate >= 14 && this.record.hitRate < 15) {
        this.score[this.config.articleType].hitRate14++;
      } else if (this.record.hitRate >= 15 && this.record.hitRate < 16) {
        this.score[this.config.articleType].hitRate15++;
      }

      // CODE LENGTH FILTER
      if (this.record.codeLength >= 0 && this.record.codeLength < 2) {
        this.score[this.config.articleType].codeLength1++;
      } else if (this.record.codeLength >= 2 && this.record.codeLength < 3) {
        this.score[this.config.articleType].codeLength2++;
      } else if (this.record.codeLength >= 3 && this.record.codeLength < 4) {
        this.score[this.config.articleType].codeLength3++;
      } else if (this.record.codeLength >= 4 && this.record.codeLength < 5) {
        this.score[this.config.articleType].codeLength4++;
      } else if (this.record.codeLength >= 5 && this.record.codeLength < 6) {
        this.score[this.config.articleType].codeLength5++;
      } else if (this.record.codeLength >= 6 && this.record.codeLength < 7) {
        this.score[this.config.articleType].codeLength6++;
      } else if (this.record.codeLength >= 7 && this.record.codeLength < 8) {
        this.score[this.config.articleType].codeLength7++;
      } else if (this.record.codeLength >= 8 && this.record.codeLength < 9) {
        this.score[this.config.articleType].codeLength8++;
      } else if (this.record.codeLength >= 9 && this.record.codeLength < 10) {
        this.score[this.config.articleType].codeLength9++;
      } else if (this.record.codeLength >= 10 && this.record.codeLength < 11) {
        this.score[this.config.articleType].codeLength10++;
      }

      // SPEED FILTER
      if (this.record.speed >= 0 && this.record.speed < 60) {
        this.score[this.config.articleType].speed30++;
      } else if (this.record.speed >= 60 && this.record.speed < 90) {
        this.score[this.config.articleType].speed60++;
      } else if (this.record.speed >= 90 && this.record.speed < 120) {
        this.score[this.config.articleType].speed90++;
      } else if (this.record.speed >= 120 && this.record.speed < 150) {
        this.score[this.config.articleType].speed120++;
      } else if (this.record.speed >= 150 && this.record.speed < 180) {
        this.score[this.config.articleType].speed150++;
      } else if (this.record.speed >= 180 && this.record.speed < 210) {
        this.score[this.config.articleType].speed180++;
      } else if (this.record.speed >= 210 && this.record.speed < 240) {
        this.score[this.config.articleType].speed210++;
      } else if (this.record.speed >= 240 && this.record.speed < 270) {
        this.score[this.config.articleType].speed240++;
      } else if (this.record.speed >= 270 && this.record.speed < 300) {
        this.score[this.config.articleType].speed270++;
      } else if (this.record.speed >= 300 && this.record.speed < 330) {
        this.score[this.config.articleType].speed300++;
      } else if (this.record.speed >= 330 && this.record.speed < 360) {
        this.score[this.config.articleType].speed330++;
      } else if (this.record.speed >= 360 && this.record.speed < 390) {
        this.score[this.config.articleType].speed360++;
      } else if (this.record.speed >= 390 && this.record.speed < 420) {
        this.score[this.config.articleType].speed390++;
      }
    ...
    updateInfo() {
      ...
      if (!this.isStarted && !this.isFinished) {
        ...
        this.record.hitRate = Number(
          ((allKeyCount / this.duration) * 1000).toFixed(2),
        );
        ...
        if (this.correctWordsCount) {
          this.record.codeLength = Number(
            (allKeyCount / this.correctWordsCount).toFixed(2),
          );
        } else {
        ...
        $(".standalone-speed-info .count-key-length").innerText =
          this.record.codeLength;
        $(".standalone-speed-info .count-key-rate").innerText =
          this.record.hitRate;
        $(".standalone-speed-info .count-key-backspace").innerText =
          this.keyCount.backspace;
      }
      ...
      // SCORE
      $(".score-info .title").innerText = ArticleType.getTypeNameWith(
        this.config.articleType,
      );

      let currentArticleTypeScore = this.score[this.config.articleType];
      $(".score-info-item.sum-words .score").innerText =
        currentArticleTypeScore.wordCount.toFixed(0);
      $(".score-info-item.sum-key .score").innerText =
        currentArticleTypeScore.keyCount.toFixed(0);
      $(".score-info-item.sum-time .score").innerText =
        (currentArticleTypeScore.timeCost / 1000).toFixed(0) + "s";

      $(".score-info-item.speed-min .score").innerText =
        currentArticleTypeScore.speedMin.toFixed(1);
      $(".score-info-item.speed-max .score").innerText =
        currentArticleTypeScore.speedMax.toFixed(1);
      $(".score-info-item.speed-ave .score").innerText =
      ...
      $(".score-info-item.hitrate-ave .score").innerText =
        currentArticleTypeScore.hitRateAve.toFixed(1);

      $(".score-info-item.code-length-min .score").innerText =
        currentArticleTypeScore.codeLengthMin.toFixed(1);
      $(".score-info-item.code-length-max .score").innerText =
        currentArticleTypeScore.codeLengthMax.toFixed(1);
      $(".score-info-item.code-length-ave .score").innerText =
        currentArticleTypeScore.codeLengthAve.toFixed(1);
      ...
      hitRateScoreArray.forEach((hitRateScore, index) => {
        ...
        $(
          `.score-statistics-item.hitrate.level-${suffix} .process`,
        ).style.backgroundColor = generateColorForChart(hitRate, 0, hitRateMax);
        if (hitRateMax === 0) {
          $(
            `.score-statistics-item.hitrate.level-${suffix} .process`,
          ).style.height = 0;
          ...
          $(
            `.score-statistics-item.hitrate.level-${suffix} .process`,
          ).style.height = `${(hitRate * HEIGHT_BAR) / hitRateMax}px`;
        }
      });
      ...
      codeLengthScoreArray.forEach((hitRateScore, index) => {
        ...
        $(
          `.score-statistics-item.codelength.level-${suffix} .process`,
        ).style.backgroundColor = generateColorForChart(
          codeLength,
          0,
          codeLength,
        );
        if (codeLengthMax === 0) {
          ...
          $(
            `.score-statistics-item.codelength.level-${suffix} .process`,
          ).style.height = `${(codeLength * HEIGHT_BAR) / codeLengthMax}px`;
        }
      });
      ...
      speedScoreArray.forEach((speedScore, index) => {
        ...
        $(
          `.score-statistics-item.speed.level-${suffix} .process`,
        ).style.backgroundColor = generateColorForChart(speed, 0, speedMax);
        if (speedMax === 0) {
          $(
            `.score-statistics-item.speed.level-${suffix} .process`,
          ).style.height = 0;
          ...
          $(
            `.score-statistics-item.speed.level-${suffix} .process`,
          ).style.height = `${(speed * HEIGHT_BAR) / speedMax}px`;
        }
      });
    }
